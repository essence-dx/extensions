(function registerDxPremierePlugin(global) {
  const { entrypoints } = require("uxp");
  const { DX_PREMIERE_MESSAGES, messageTypeForEntrypoint } = global.dxPremiereMessages;
  const { DX_PREMIERE_COMMAND_PLANS, commandPlanForMessage } = global.dxPremiereCommandPlans;
  ...
  function bindPanelControls(root) {
    ...
    for (const button of panelRoot.querySelectorAll("[data-command]")) {
      button.addEventListener("click", () => {
        if (button.dataset.command === DX_PREMIERE_MESSAGES.searchMediaAssets) {
          runMediaSearchFromPanel();
          return;
        }
  ...
  function runMediaSearchFromPanel() {
    const plan = DX_PREMIERE_COMMAND_PLANS.searchMediaAssets;
    const query = document.querySelector("[name='media-query']")?.value?.trim() ?? "";
    ...
    panels: {
      dxCommandCenterPanel: {
        show(event) {
          ...
          handleMessage(DX_PREMIERE_MESSAGES.showStatus);
        }
      }
    }
  });
