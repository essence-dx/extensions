# Office Word Add-in Starter

This is the source-work evidence record for `hosts/office/dx-word`.

## Official Evidence

- Word add-ins overview: https://learn.microsoft.com/en-us/office/dev/add-ins/word/word-add-ins-programming-overview
- Word requirement sets: https://learn.microsoft.com/en-us/javascript/api/requirement-sets/word/word-api-requirement-sets
...
Word Office Add-ins are web add-ins. The DX adapter therefore uses an
add-in-only XML `TaskPaneApp` manifest, a `Document` host declaration, a minimal
`WordApi` requirement, and an HTTPS task-pane source placeholder. It must not
spawn DX locally, use native messaging, request broad document write permission,
or mutate document text before loaded-host proof exists.

The first approved actions are source-level typed plans only:

- show DX status through a future local-service bridge
- search DX assets and document snippets through a future local-service bridge
...
claim sideloaded Word, signing, AppSource approval, or public release readiness.

## Deferred Proof

The local-service proof remains deferred. Sideloaded Word smoke, hosted
