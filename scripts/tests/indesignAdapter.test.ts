...
const adapterRoot = join(root, "hosts", "adobe", "dx-indesign-uxp");
...
const starterGuidePath = join(root, "docs", "indesign-uxp-plugin-starter.md");
...
const forbiddenInDesignMutationPattern =
  /require\(["']indesign["']\)|\bapp\.activeDocument\b|\bapp\.documents\b|\bdocuments\.(?:add|open)\b|createDocument|openDocument|saveDocument|saveAs|exportFile|packageForPrint|place\(|pages\.add|spreads\.add|textFrames\.add|rectangles\.add|ovals\.add|polygons\.add|stories\.add|paragraphStyles\.add|characterStyles\.add|objectStyles\.add|swatches\.add|contents\s*=|insertionPoints|parentStory|hybrid|native|require\(["'][^"']+\.node["']\)|addon/i;
...
assert.match(registrySource, /id = "dx\.indesign\.command-center"/);
assert.match(registrySource, /path = "hosts\/adobe\/dx-indesign-uxp"/);
assert.match(registrySource, /manifest = "hosts\/adobe\/dx-indesign-uxp\/dx\.extension\.toml"/);
assert.match(registrySource, /status = "experimental"/);

const packageJson = JSON.parse(readFileSync(join(root, "package.json"), "utf8"));
assert.equal(
  packageJson.scripts["test:indesign-adapter"],
  "node --experimental-strip-types scripts/tests/indesignAdapter.test.ts"
);
assert.equal(
  (packageJson.workspaces ?? []).includes("hosts/adobe/dx-indesign-uxp"),
  false,
  "InDesign adapter must stay a root-tested source adapter, not an npm workspace"
);
...
assert.equal(lockedWorkspaces.includes("hosts/adobe/dx-indesign-uxp"), false);
assert.equal(
  Object.keys(lockfile.packages ?? {}).some((path) => path.includes("dx-indesign-uxp")),
  false
);

const testJ1Source = readFileSync(join(root, "scripts", "test-j1.ps1"), "utf8");
assert.match(testJ1Source, /Invoke-DxCommand "npm" @\("run", "test:indesign-adapter"\)/);
...
assert.equal(dxManifest.sections.extension.id, "dx.indesign.command-center");
assert.equal(dxManifest.sections.extension.name, "DX InDesign Command Center");
assert.equal(dxManifest.sections.extension.official, true);
assert.deepEqual(dxManifest.sections.compatibility.hosts, ["indesign"]);
...
assert.equal(capabilityIds.includes("adobe.indesign.document.write"), false);
...
assert.equal(
  receipt.latest_path,
  ".dx/receipts/extensions/dx.indesign.command-center/host-action-index-latest.json"
);
...
assert.equal(uxpManifest.id, "dx.indesign.command-center.development");
...
assert.deepEqual(uxpManifest.host, { app: "ID", minVersion: "18.5.0" });
assert.deepEqual(
...
assert.doesNotMatch(forbiddenManifestText, /launchProcess|openExternal|openPath|localFileSystem|webview|clipboard|enableUserInfo|ipc|native|addon/i);

const messagesSource = readFileSync(messagesPath, "utf8");
assert.match(messagesSource, /DX_INDESIGN_MESSAGES[\s\S]*showStatus:\s*"dx\.indesign\.show_status"/);
assert.match(messagesSource, /searchAssets:\s*"dx\.indesign\.search_assets"/);
assert.match(messagesSource, /showReceipts:\s*"dx\.indesign\.show_receipts"/);
assert.match(messagesSource, /DX_INDESIGN_ENTRYPOINTS[\s\S]*panel:\s*"dxCommandCenterPanel"/);
...
assert.match(commandPlansSource, /mutatesInDesignDocument:\s*false/);
...
assert.match(pluginSource, /DX_INDESIGN_COMMAND_PLANS/);
...
assert.doesNotMatch(pluginSource, forbiddenInDesignMutationPattern);
...
assert.match(indexSource, /data-command="dx\.indesign\.show_status"/);
assert.match(indexSource, /data-command="dx\.indesign\.search_assets"/);
assert.match(indexSource, /data-command="dx\.indesign\.show_receipts"/);
...
assert.match(readmeSource, /loaded InDesign UXP smoke/i);
...
assert.match(adobeBoundarySource, /InDesign owns document, page, and typography\s+behavior/i);

const starterGuideSource = readFileSync(starterGuidePath, "utf8");
assert.match(starterGuideSource, /https:\/\/developer\.adobe\.com\/indesign\/uxp\/plugins\//);
assert.match(starterGuideSource, /https:\/\/developer\.adobe\.com\/indesign\/uxp\/plugins\/concepts\/manifest\//);
assert.match(starterGuideSource, /https:\/\/developer\.adobe\.com\/indesign\/uxp\/plugins\/concepts\/entry-points\//);
assert.match(starterGuideSource, /host\.app.*ID/i);
assert.match(starterGuideSource, /minVersion.*18\.5\.0/i);
...
assert.match(professionalTargetsSource, /id = "adobe\.indesign\.uxp"[\s\S]*recommended_priority = "active-scaffold"/);
assert.match(professionalTargetsSource, /id = "adobe\.indesign\.uxp"[\s\S]*loaded-host smoke receipt/);

console.log("InDesign adapter verified");
