...
try {
    $env:DX_VERIFICATION_COMMAND = "npm run preflight:loaded-host-targets:j1"
    Invoke-DxCommand "npm" @("run", "test:loaded-host-preflight-receipts")
    Invoke-DxCommand "node" @("--experimental-strip-types", "scripts/write-loaded-host-preflight-receipts.ts")
