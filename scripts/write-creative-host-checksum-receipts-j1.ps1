...
try {
    $env:DX_VERIFICATION_COMMAND = "npm run write:creative-host-checksum-receipts:j1"
    Invoke-DxCommand "npm" @("run", "test:creative-host-checksum-receipts")
    Invoke-DxCommand "node" @("--experimental-strip-types", "scripts/write-creative-host-checksum-receipts.ts")
